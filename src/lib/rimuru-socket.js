...
import * as starcore from "@itsliaaa/starcore";
...
sock.sendRich = async (jid, elements, quoted, options) => {
    try {
      // 1. Si la librería exporta sendRich directamente
      if (typeof starcore.sendRich === "function") {
        return await starcore.sendRich(sock, jid, elements, quoted, options);
      }
      // 2. Si la librería funciona como un wrapper que devuelve un socket vitaminado
      const wrapFn = starcore.default || starcore.makeWASocket || starcore;
      if (typeof wrapFn === "function") {
         const wrappedSock = wrapFn(sock);
         if (wrappedSock && typeof wrappedSock.sendRich === "function") {
             return await wrappedSock.sendRich(jid, elements, quoted, options);
         }
      }
      throw new Error("No se pudo detectar la función sendRich en @itsliaaa/starcore.");
    } catch (error) {
      console.error("❌ [Error en sendRich]:", error);
      throw error;
    }
  };
...
sock.sendPreview = async (jid, text, imageBuffer, options = {}) => {
