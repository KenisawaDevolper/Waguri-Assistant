...
function formatAfkDuration(ms) {
  ...
  if (days > 0) return `${days} hari ${hours % 24} horas`;
...
function checkPermission(m, pluginConfig) {
  const effectiveConfig = {
    ...pluginConfig,
    ...(pluginConfig?.commandProfiles?.[String(m.command || "").toLowerCase()] || {}),
  };
  ...
  if (effectiveConfig.isOwner && !m.isOwner && !hasAccess) {
    return {
      allowed: false,
  ...
  if (effectiveConfig.isPartner && !m.isPartner && !m.isOwner && !hasAccess) {
    return { allowed: false, reason: "🤝 Partner only!" };
  }

  const overrides = db.setting("capprem") || {};
  const isPremiumFeature = overrides[pluginConfig.name] !== undefined ? overrides[pluginConfig.name] : effectiveConfig.isPremium;
  ...
  if (effectiveConfig.isGroup && !m.isGroup) {
    return {
      allowed: false,
  ...
  if (effectiveConfig.isPrivate && m.isGroup) {
    return {
      allowed: false,
  ...
  if (effectiveConfig.isBotAdmin && m.isGroup && !m.isBotAdmin) {
    return {
      allowed: false,
      reason:
  ...
  if (m.isGroup) {
    ...
    if (group) {
      if (effectiveConfig.category === "game" && group.game === false) {
      ...
      if (effectiveConfig.category === "rpg" && group.rpg === false) {
        if (!m.isAdmin && !m.isOwner && !hasAccess) {
          return {
            allowed: false,
