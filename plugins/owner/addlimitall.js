...
async function handler(m, { sock }) {
    try {
        ...
        if (isNaN(amount) || amount <= 0) {
            return m.reply(`⚠️ *ᴄóᴍᴏ ᴜsᴀʀ* 𓈒 ◌\n\nꕥ Ingresa la cantidad de límite que deseas añadir.\n\n\`Ejemplo: ${m.prefix}addlimitall 50\` ( ᴗ͈ˬᴗ͈ )`)
        }
        ...
        if (participants.length === 0) {
            return m.reply(`❌ ꕥ No hay miembros en este grupo ( ᴗ͈ˬᴗ͈ )`)
        }
        ...
        await m.react('⚡')
        await m.reply(
           `✅ ꕥ Límite añadido a todos los miembros (Total *${successCount}* miembros) en el grupo *${gb?.subject}* 𓈒 ◌🌸`,
            )
