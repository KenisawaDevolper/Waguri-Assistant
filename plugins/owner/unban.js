...
const pluginConfig = {
    ...
    category: 'owner',
    description: 'Eliminar usuario de la lista de baneados',
...
async function handler(m, { sock }) {
    ...
    if (!targetNumber || targetNumber.length < 10 || targetNumber.length > 15) {
        return m.reply(
            ...
            `\`Ejemplo: ${m.prefix}unban 6281234567890\` ( ᴗ͈ˬᴗ͈ )`
        )
    }
    ...
    if (index === -1) {
        return m.reply(`❌ ꕥ El número \`${targetNumber}\` no está en la lista de baneados 𓈒 ◌`)
    }
