...
const pluginConfig = {
    ...
    category: 'sticker',
    description: 'Crea stickers a partir de imágenes o videos con opciones personalizadas estilo Waguri',
...
async function handler(m, { sock, config: botConfig }) {
    ...
    if (!isImage && !isVideo) {
        await m.reply(
            `ꕥ 𝖢𝖱𝖤𝖠𝖣𝖮𝖱 𝖣𝖤 𝖲𝖳𝖨𝖢𝖪𝖤𝖱𝖲 ｡ﾟ+.ღ(ゝ◡ ⚈᷀᷁ღ)\n\n` +
            `      • Uso :: Responde a una imagen o video con:\n` +
            `        \`${m.prefix}s\`\n\n` +
            `      𓈒 ◌ㅤ──    *𝖮𝖯𝖢𝖨𝖮𝖭𝖤𝖲*\n` +
            `      • \`--crop\` :: Recortar en formato cuadrado\n` +
            `      • \`--resize WxH\` :: Ajustar tamaño (ej: 256x256)\n` +
            ...
            `      • \`${m.prefix}s --circle\`\n` +
            `      • \`${m.prefix}s NombrePaquete Autor\`\n\n` +
            `> 𝗐⍺𝗀𝗎ɾɩ ⍺𝗌𝗌ı𝗌ƚ⍺𝗇ƚ ツ`
        )
        return
    }
    ...
    try {
        ...
        if (!buffer) {
            await m.reply(
                `ꕥ Error al descargar el archivo multimedia.\n\n` +
                `> 𝗐⍺𝗀𝗎ɾɩ ⍺𝗌𝗌ı𝗌ƚ⍺𝗇ƚ ツ`
            )
            await m.react('❌')
            return
        ...
        if (isVideo) {
            ...
            try {
                ...
                if (duration > 10) {
                    await m.reply(
                        `ꕥ 𝖵𝖨𝖣𝖤𝖮 𝖣𝖤𝖬𝖠𝖲𝖨𝖠𝖣𝖮 𝖫𝖠𝖱𝖦𝖮 ｡ﾟ+.ღ(ゝ◡ ⚈᷀᷁ღ)\n\n` +
                        `      • Duración :: ${duration.toFixed(1)} segundos\n` +
                        `      • Máximo permitido :: 10 segundos\n\n` +
                        `> 𝗐⍺𝗀𝗎ɾɩ ⍺𝗌𝗌ı𝗌ƚ⍺𝗇ƚ ツ`
                    )
                    await m.react('☢')
        ...
        const packname = options.packname || botConfig.sticker?.packname || botConfig.bot?.name || 'Waguri Assistant'
