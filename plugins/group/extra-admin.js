...
const pluginConfig = {
  ...
  category: "group",
  description: "Utilitas admin grup tambahan yang no tersedia di Rimuru.",
  usage: ".<command>",
  example: ".admincount",
  isGroup: true,
  isAdmin: true,
...
function formatAge(ts){
  ...
  if(ms<0)return "Aún no diketahui";
  ...
  try{
    ...
    switch(c){
      ...
      case "votekick":{
        ...
        const store=db?.data?.voteExpulsa||{};
        db.data.voteExpulsa=store;
        store[m.chat]??={};
        store[m.chat][target]??=[];
...
${gd.motd.text}`);
      ...
      case "previewwelcome":{
        ...
        if(!raw)return m.reply("ℹ️ Bienvenida message belum diatur.");
        const meta3=await sock.groupMetadata(m.chat);
        const num=(m.sender||"").split("@")[0];
        const rendered=String(raw).replace(/\{user\}/g,`@${num}`).replace(/\{name\}/g,`@${num}`).replace(/\{group\}/g,meta3.subject||"Grupo").replace(/\{num\}/g,num);
        return m.reply(`👋 *Preview Bienvenida*

${rendered}`,{mentions:[m.sender]});
      }
      case "lockinfo":
        await sock.groupSettingUpdate(m.chat,"locked");
        return m.reply("🔒 Info grup dikunci. Solo admins yang dapat mengubah info grup.");
      case "unlockinfo":
      ...
      case "ephemeral":{
        ...
        if(typeof sock.groupToggleEphemeral!=="function")return m.reply("❌ Baileys pada versi ini no menyediakan groupToggleEphemeral.");
        await sock.groupToggleEphemeral(m.chat,seconds);
        return m.reply(seconds===0?"🕊️ Mensaje sementara dimatikan.":`🕊️ Mensaje sementara: *${seconds/86400} hari*`);
      }
      case "seticon":{
        if(!m.isImage&&!m.quoted?.isImage)return m.reply("📷 Envía/reply gambar lalu gunakan .seticon");
        const q=m.isImage?m:m.quoted;
        const buffer=await q.download();
        if(!buffer)return m.reply("❌ Falló mengambil gambar.");
        if(typeof sock.updateProfilePicture!=="function")return m.reply("❌ API update foto grup no tersedia.");
        await sock.updateProfilePicture(m.chat,buffer);
        return m.reply("✅ Foto grup berhasil diganti.");
      }
      case "delicon":{
        if(typeof sock.removeProfilePicture!=="function")return m.reply("❌ API hapus foto grup no tersedia pada versi Baileys ini.");
        await sock.removeProfilePicture(m.chat);
        return m.reply("✅ Foto grup berhasil dihapus.");
      }
      default:return m.reply("❌ Command no dikenal.");
    }
  }catch(e){
    console.error("[group-extra]",e);
    return m.reply("❌ Falló menjalankan fitur. Pastikan bot masih admin dan API WhatsApp mendukung operasi ini.");
  }
}
