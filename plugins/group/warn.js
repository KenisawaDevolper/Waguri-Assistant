...
async function handler(m, { sock }) {
    ...
    if (!args[0] && !m.quoted && (!m.mentionedJid || m.mentionedJid.length === 0)) {
        return m.reply(
            ...
            `      ⚠️   𓈒 ◌ㅤ──    *𝖫í𝗆ıƚᧉ*: ${maxWarns} (𝖪ı𝖼𝗄 ⍺𝗎ƚ𝗈𝗆áƚı𝖼𝗈)\n\n` +
            `(•ૢ⚈͒⌄⚈͒•ૢ) *𝖢𝖮𝖬𝖠𝖭𝖣𝖮𝖲* 🌸\n` +
            `      🔘   𓈒 ◌ㅤ──    \`${m.prefix}warn @user <motivo>\`\n` +
            `      🔘   𓈒 ◌ㅤ──    \`${m.prefix}warn max <número>\`\n` +
            `      🔘   𓈒 ◌ㅤ──    \`${m.prefix}listwarn\`\n` +
            `      🔘   𓈒 ◌ㅤ──    \`${m.prefix}resetwarn @user\`\n\n` +
            `> 💡 *𝖳ı𝗉:* 𝖲ı ᧉ𝗅 𝗎𝗌𝗎⍺𝗋ı𝗈 ⍺𝗅𝖼⍺𝗇𝗓⍺ ᧉ𝗅 𝗅í𝗆ıƚᧉ (${maxWarns}), 𝗌ᧉ𝗋á ᧉ𝗑𝗉𝗎𝗅𝗌⍺𝖽𝗈 ⍺𝗎ƚ𝗈𝗆áƚı𝖼⍺𝗆ᧉ𝗇ƚᧉ.\n\n` +
    ...
    if (warnCount >= maxWarns) {
        try {
            ...
            await m.reply(
                ...
                `> ❝ ¡𝖧⍺ ⍺𝗅𝖼⍺𝗇𝗓⍺𝖽𝗈 ᧉ𝗅 𝗅í𝗆ıƚᧉ 𝖽ᧉ *${maxWarns}* ⍺𝖽᥎ᧉ𝗋ƚᧉ𝗇𝖼ı⍺𝗌 𝗒 𝖿𝗎ᧉ ᧉ𝗅ı𝗆ı𝗇⍺𝖽𝗈/⍺ 𝖽ᧉ𝗅 𝗀𝗋𝗎𝗉𝗈! ❞\n\n` +
                `( ᴗ͈ˬᴗ͈ ) 𝑊⍺ց𝗎𝗋ı 𝖠𝗌𝗌ı𝗌ƚ⍺𝗇ƚ`,
                { mentions: [targetUser] }
            )
        ...
        await m.reply(
            ...
            `⚠️ *𝖠𝖣𝖵𝖤𝖱𝖳𝖤𝖭𝖢𝖨𝖠 𝖣𝖠𝖣𝖠*\n\n` +
            `      👤   𓈒 ◌ㅤ──    *𝖴𝗌𝗎⍺𝗋ı𝗈* : @${targetName}\n` +
            `      🚨   𓈒 ◌ㅤ──    *𝖨𝗇𝖿𝗋⍺𝖼𝖼ıó𝗇* : ${warnCount}/${maxWarns}\n` +
            `      📝   𓈒 ◌ㅤ──    *𝖬𝗈ƚı᥎𝗈* : ${reason}\n\n` +
            `> ❝ ¡𝖳ᧉ 𝗊𝗎ᧉ𝖽⍺𝗇 *${maxWarns - warnCount}* ⍺𝖽᥎ᧉ𝗋ƚᧉ𝗇𝖼ı⍺𝗌 ⍺𝗇ƚᧉ𝗌 𝖽ᧉ 𝗌ᧉ𝗋 ᧉ𝗑𝗉𝗎𝗅𝗌⍺𝖽𝗈/⍺! ❞\n\n` +
            `( ᴗ͈ˬᴗ͈ ) 𝑊⍺ց𝗎𝗋ı 𝖠𝗌𝗌ı𝗌ƚ⍺𝗇ƚ`,
            { mentions: [targetUser] }
        )
    }
