...
const pluginConfig = {
    ...
    category: 'main',
    description: 'Muestra el rendimiento y estado del sistema en modo texto con estilo Waguri',
    usage: '.ping2',
    example: '.ping2',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    try {
        ...
        const tableData = [
            ['Latencia WhatsApp', `${waRoundtrip} ms`],
            ['Tiempo de Respuesta', `${totalExec} ms`],
            ['Estado del Sistema', 'Activo'],
            ['Nombre de Host', os.hostname()],
            ['Plataforma', `${os.platform()} (${os.arch()})`],
            ['Motor Node.js', process.version],
            ['Procesador CPU', `${cpus[0]?.model?.slice(0, 25)}`],
            ['Núcleos CPU', `${cpus.length} Core(s)`],
            ['Carga de CPU', `${cpuPct}%`],
        ...
        await sock.sendTable(
            m.chat,
            'ꕥ RENDIMIENTO Y SISTEMA ꕥ',
            ['Métrica', 'Valor'],
            tableData,
            m,
            {
                headerText: `ꕥ 𝖤𝖲𝖳𝖠𝖣𝖮 𝖣𝖤𝖫 𝖲𝖨𝖲𝖳𝖤𝖬𝖠 ｡ﾟ+.ღ(ゝ◡ ⚈᷀᷁ღ)\n\nଘ៸៸᳐⦁⩊⦁៸៸᳐ଓ « Estadísticas del servidor en tiempo real »`,
